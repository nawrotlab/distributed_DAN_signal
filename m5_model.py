...
def build_model(params):
    ...
    # values randomly drawn in each call of parallel_run()
    baseline = 0.0
    learning_rate = 0.0

    w_MBONp_DAN = Uniform(params['w_MBONp_DAN'][0], params['w_MBONp_DAN'][1]).get_value()
    ...
    # neuron templates
    # MBON
    #### IMPORTANT: mp (nomally membran potential is the calcium signal here --> therfor there is no transfer function). Annarchy looks for keyword mp and r i equation during model compilation
    MBON_temp = Neuron(
        parameters="""
    ...
    # Model 5: Mixed inhibitory/excitatory feedback MBON - DAN_PAM
    weight_matrix = np.array([-w_MBONp_DAN*1.1, -w_MBONp_DAN*1.1, w_MBONp_DAN, w_MBONp_DAN]).reshape(1, 4)  # c1, d1, f1, g1
    syns['MBONp_DANPAM'] = Projection(pre=pops['MBONp'], post=pops['DAN_PAM'], target='exc')
    syns['MBONp_DANPAM'].connect_from_matrix(weight_matrix)
